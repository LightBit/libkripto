...
int main(void)
{
	...
	kripto_hash_all(kripto_hash_blake256, 0, 0, 0, "\x0", 1, hash, 28);
	for(i = 0; i < 28; i++) printf("%.2x", hash[i]);
	putchar('\n');

	/* 256 */
	puts("7576698ee9cad30173080678e5965916adbb11cb5245d386bf1ffda1cb26c9d7");
	kripto_hash_all(kripto_hash_blake256, 0, 0, 0, "The quick brown fox jumps over the lazy dog", 43, hash, 32);
	for(i = 0; i < 32; i++) printf("%.2x", hash[i]);
	putchar('\n');

	puts("0ce8d4ef4dd7cd8d62dfded9d4edb0a774ae6a41929a74da23109e8f11139c87");
	kripto_hash_all(kripto_hash_blake256, 0, 0, 0, "\x0", 1, hash, 32);
	for(i = 0; i < 32; i++) printf("%.2x", hash[i]);
	putchar('\n');
