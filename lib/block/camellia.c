...
/* Derived from Tom St Denis implementation */
...
static const uint32_t SP1110[256] =
{
...
static const uint32_t SP0222[256] =
{
...
static const uint32_t SP3033[256] =
{
...
static const uint32_t SP4404[256] =
{
...
)
{
	...
	if(i < 16) for(j = i + 8; i < j; i++) t[i] = ~t[i - 8];

	if(key_len <= 16) while(i < 32) t[i++] = 0;
	else
	{
		while(i < 24) t[i++] = 0;

		if(i == 24) for(j = i + 8; i < j; i++) t[i] = ~t[i - 8];
