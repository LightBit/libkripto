...
/* Based on Brian Gladman's and Wei Dai's implementations */
...
#define S(X) TS[(X) & 511]
#define S0(X) TS[(X) & 255]
#define S1(X) TS[256 + ((X) & 255)]

#define FORW_MIX(A, B, C, D)	\
{				\
	B ^= S0(A);		\
	B += S1(A >> 8);	\
	C += S0(A >> 16);	\
...
static void mars_encrypt(const kripto_block *s, const void *pt, void *ct)
{
	uint32_t a;
	uint32_t b;
	uint32_t c;
	...
	a = LOAD32L(CU8(pt)     ) + s->k[0];
	b = LOAD32L(CU8(pt) +  4) + s->k[1];
	c = LOAD32L(CU8(pt) +  8) + s->k[2];
	d = LOAD32L(CU8(pt) + 12) + s->k[3];
	...
	STORE32L(a - s->k[36], U8(ct)     );
	STORE32L(b - s->k[37], U8(ct) +  4);
	STORE32L(c - s->k[38], U8(ct) +  8);
	STORE32L(d - s->k[39], U8(ct) + 12);
}

static void mars_decrypt(const kripto_block *s, const void *ct, void *pt)
{
	uint32_t a;
	uint32_t b;
	uint32_t c;
	...
	d = LOAD32L(CU8(ct)     ) + s->k[36];
	c = LOAD32L(CU8(ct) +  4) + s->k[37];
	b = LOAD32L(CU8(ct) +  8) + s->k[38];
	a = LOAD32L(CU8(ct) + 12) + s->k[39];
	...
	STORE32L(d - s->k[0], U8(pt)     );
	STORE32L(c - s->k[1], U8(pt) +  4);
	STORE32L(b - s->k[2], U8(pt) +  8);
	STORE32L(a - s->k[3], U8(pt) + 12);
}

static kripto_block *mars_recreate
(
	kripto_block *s,
	unsigned int r,
	const void *key,
...
)
{
	...
	{
		for(i = 0; i < 15; i++)
			t[i] ^= ROL32_03(t[(i + 8) % 15]
				^ t[(i + 13) % 15])
				^ (4 * i + j);

		for(x = 0; x < 4; x++)
			for(i = 0; i < 15; i++)
				t[i] = ROL32_09(t[i] + S(t[(i + 14) % 15]));
	...
	for(i = 5; i < 37; i += 2)
	{
		...
		w ^= ROL32(TS[265 + (s->k[i] & 3)], s->k[i - 1]) & m;
...
static kripto_block *mars_create
(
	unsigned int r,
	const void *key,
	unsigned int key_len
)
{
	...
	(void)mars_recreate(s, r, key, key_len);

	return s;
}

static void mars_destroy(kripto_block *s)
{
	kripto_memory_wipe(s, sizeof(kripto_block));
	free(s);
}

static const kripto_block_desc mars =
{
	&mars_create,
	&mars_recreate,
	0, /* tweak */
