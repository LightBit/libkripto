...
static void tiger_process(kripto_hash *s, const void *data)
{
	uint64_t x0;
	uint64_t x1;
	uint64_t x2;
	...
	x0 = LOAD64L(CU8(data));
	x1 = LOAD64L(CU8(data) + 8);
	x2 = LOAD64L(CU8(data) + 16);
	x3 = LOAD64L(CU8(data) + 24);
	x4 = LOAD64L(CU8(data) + 32);
	x5 = LOAD64L(CU8(data) + 40);
	x6 = LOAD64L(CU8(data) + 48);
	x7 = LOAD64L(CU8(data) + 56);
...
static kripto_hash *tiger_recreate
(
	kripto_hash *s,
	unsigned int r,
	size_t len
)
{
	...
	s->h[0] = 0x0123456789ABCDEF;
	s->h[1] = 0xFEDCBA9876543210;
	s->h[2] = 0xF096A5B4C3B2E187;

	return s;
}

static void tiger_input
(
	kripto_hash *s,
	const void *in,
	size_t len
